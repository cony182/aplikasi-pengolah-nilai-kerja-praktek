...
const Login = () => {
   ...
   const [errorInput, setErrorInput] = useState(""); // state warning jika input field kosong
   const [incorrectCredentials, setIncorrectCredentials] = useState(false); // state warning pada input field tidak ada di DB
   const [processing, setProccessing] = useState(false); // state loading pada button
   ...
   const login = (e) => {
      ...
      if (email == "" || password == "") {
         ...
         return setErrorInput("Silakan isi data dengan benar");
      }
      ...
      axios
         ...
         .then((response) => {
            if (response.status == 202) return navigate("/email/send/" + response.data.url);
         ...
         .catch((err) => {
            ...
            if (err.response.status === 404) {
               ...
               setErrorInput("Email atau password salah");
            }
   ...
   const googleLoginSuccess = (res) => {
      axios
         ...
         .catch((err) => {
            console.log(err);
            navigate("/error/login/google/auth");
         });
   };

   const googleLoginFailure = (res) => {
      navigate("/error/login/google/auth=failure");
   };
   ...
   return loading ? (
      ...
      <>
         <div className="flex items-center justify-center bg-primaryLight h-screen -mt-16">
            <div
               className={
                  incorrectCredentials
                     ? "rounded-xl border border-red-500 border-stroke w-full md:w-3/4 xl:w-3/4 bg-primaryLight shadow-default dark:border-strokedark dark:bg-boxdark"
                     : "rounded-xl border  border-stroke w-full md:w-3/4 xl:w-3/4 bg-primaryLight shadow-default dark:border-strokedark dark:bg-boxdark"
               }
            >
                  ...
                  <div className="w-full border-stroke dark:border-strokedark xl:w-1/2 xl:border-l-2">
                     <div className="w-full p-4 sm:p-12.5 xl:p-17.5">
                        ...
                        <form onSubmit={login}>
                           ...
                           <div className="mb-5 text-right mr-3 text-sm text-blue-500 underline">
                        ...
                        <div className="mt-6 text-center">
                           <p className="text-sm">
                              Tidak punya akun?{" "}
                              <Link to="/register" className="text-primary underline text-blue-800">
                                 Registrasi
                              </Link>
                           </p>
                        </div>
