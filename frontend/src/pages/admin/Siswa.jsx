...
const Siswa = () => {
   ...
   useEffect(() => {
      axios
         .get(`/admin/students?search=${search}&limit=${limit}&lastId=${lastId}`, { withCredentials: true })
         .then((response) => {
            console.log(response.data);
            const newSiswa = response.data.siswa;
            ...
            <ol className="inline-flex items-center space-x-1 md:space-x-3">
               ...
               <li aria-current="page">
                  <div className="flex items-center">
                     ...
                     <span className="ml-1 text-sm font-medium text-gray-500 md:ml-2 dark:text-gray-400">Siswa</span>
                  </div>
               </li>
            </ol>
         ...
         <div className="mt-4 mx-4 bg-gray-800">
            <div className="w-full overflow-hidden rounded-lg shadow-xs">
               <div className="w-full overflow-x-auto">
                  <div className="text-gray-300 ml-3 mb-2 font-bold mt-4">Daftar semua siswa</div>
                  <InfiniteScroll
                     dataLength={siswa.length}
                     next={fetchUsers}
                     hasMore={hasMore}
                                    ...
                                    <div className="flex items-center text-sm">
                                       <div className="relative hidden w-8 h-8 mr-3 rounded-full md:block">
                                          <img
                                             ...
                                             src={item.user.picture ? `${axios.defaults.baseURL}/images/avatar/${item.user.picture}` : Image}
                                       ...
                                       <div>
                                          ...
                                          <p className="text-xs text-gray-600 dark:text-gray-400">{item.NIS}</p>
                                       </div>
                                    </div>
                                 </td>
                                 <td className="px-4 py-3 text-sm relative">{item.kela.nama_kelas ? item.kela.nama_kelas : "Kosong"}</td>
                                 <td className="px-4 py-3 text-xs">{item.wali_kelas.length >= 1 ? item.wali_kelas[0].guru.nama : "kosong"}</td>
